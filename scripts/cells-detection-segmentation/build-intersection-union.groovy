/**
 * Create intersection/union annotations between two object classes.
 * Works with detections (cells) and annotations.
 *
 * Expected workflow:
 * - Run once on one image to validate class names.
...
// Output classes
// Keep outputs method-specific to avoid overwriting.
...
def gtObjects = getObjectsByClass(gt_class)
def predObjects = getObjectsByClass(pred_class)

if (gtObjects.isEmpty()) {
    print("No object found for class '${gt_class}'.")
    return
}
if (predObjects.isEmpty()) {
    print("No object found for class '${pred_class}'.")
    return
}
